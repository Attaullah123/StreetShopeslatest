...
import Fragments.StoreProductDetailsFragment;
import Fragments.StoreProductItemDetailsFragment;
...
public class ProductReviewsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ProductReviewsAdapter(Context context, List<String> list) {
    ...
    class FooterViewHolder extends RecyclerView.ViewHolder {
        ...
        public void addReview(final String name ,final String description,final String rate)
        {
            ...
            progress.setMessage("Loading Product Reviews");
            progress.show();
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        String url = "http://streetshops.sg/dashboard/appapi/addProductReview.php";
                        ...
                        KlientNameValuePairs.add(new BasicNameValuePair("productid", StoreProductItemDetailsFragment.productid));
                                ...
                                if(responsee.equals("1"))
                                {
                                    new AlertDialog.Builder(context)
                                            ...
                                            .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                                                ...
                                                {
                                                    //Fragment frg = null;
                                                    //frg = MainActivity.manager.f.findFragmentByTag("StoreProductDetailsFragment");
                                                    final FragmentTransaction ft = MainActivity.manager.beginTransaction();
                                                    ft.detach(new StoreProductDetailsFragment());
                                                    ft.attach(new StoreProductDetailsFragment());
                                                    ft.commit();
                                                }
                                            })
                                            .setIcon(android.R.drawable.ic_dialog_alert)
